# Historical Product Planning and Design Source

> **Documentation Governance**
> - **Role:** Historical source preserving the former combined product, design, and implementation planning document.
> - **Authority:** Informational — Historical Reference.
> - **Scope:** Provenance for earlier product intent, phase planning, and design evolution.
> - **Not Responsible For:** Current product requirements, architecture, Runtime Design, implementation direction, status, remediation, or acceptance.
> - **Depends On:** GOVERNANCE.DOCUMENTATION, PRODUCT.CORE
> - **Status:** HISTORICAL REFERENCE
> - **Superseded By:** `docs/PRD.md`

This archived document is retained for provenance. It is not an implementation authority;
the current Product Definition is `docs/PRD.md`, and detailed technical behavior
is owned by the Topic Authorities registered in `docs/AUTHORITY.md`.
...
该历史设计曾通过专用 Executor Adapter 实现接口；该 Adapter 已删除，当前实现只允许 Hermes 公共插件接口。
...
Phase 3 详细规范的唯一当前导航与主题归属见 `docs/AUTHORITY.md`。以下清单仅保留
为当次重新设计的冻结记录，不是完整文档索引，也不独立决定文档优先级：
...
Phase 5 开始前，必须先在 `docs/AUTHORITY.md` 注册唯一的 Evaluation Method
Topic Authority，并由该文档列出 Hermes 原生能力与各组开关状态，避免 Hermes
内置重试、Guardrail 或 Checkpoint 污染实验变量。当前尚未注册该 Topic，也不应
假定某个预设文件路径已经存在。
...
```text
已注册的 Evaluation Method Topic Authority（Phase 5 开始前建立）
...
原 Phase 3 计划记录的状态（当前实现状态统一见 `docs/PHASE_STATUS.md`）：
...
Phase 3 规范位于 `docs/adr/ADR-003-task-reliability-boundary.md` 和 `docs/phase3/`，
具体主题的唯一权威来源由 `docs/AUTHORITY.md` 登记。生产实现由现有
`Phase2Runtime → RuntimeGovernanceCore → AstraStore` 治理链承载，没有创建平行
Runtime 或 Workflow Engine。本段的完成表述属于原计划记录；当前实现声明以
`docs/PHASE_STATUS.md` 为准。历史验收 Gate、测试结果、测试命令和冻结产物不再
保留为当前指导。
